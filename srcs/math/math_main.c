...
void	calc_draw_lines(t_math_pos *data, t_img_data *img, int x)
{
	int	y;
...
int	wall_printer(t_vec2 player, t_mlx map_data)
{
	...
	while (x < WIDTH)
	{
		...
		avoid_div_0(&data);
		calc_step_dist(&data);
		dda_loop(&data, &map_data);
		calc_draw_lines(&data, &img, x);
		x++;
	}
	//TODO color ceiling and floor
	img_printed = img;
	mlx_put_image_to_window(map_data.mlx, map_data.win, img_printed.img, 0, 0);
	return (0);
}
